...
export default function PageTransition({ children, skipTransitionFor = [] }: PageTransitionProps) {
  ...
  // Only trigger transition on pathname change
  useEffect(() => {
  ...
  // Always update displayChildren if it's the first render
  useEffect(() => {
    if (!showStripe && !pendingChildren && prevPathname.current === pathname) {
      setDisplayChildren(children);
    }
    ...
    <div className="relative w-full min-h-screen overflow-x-hidden">
      <AnimatePresence mode="wait">
        {showStripe && (
          ...
          >
            <div className="relative w-full h-48 md:h-56 flex items-center">
              {/* Red stripe */}
